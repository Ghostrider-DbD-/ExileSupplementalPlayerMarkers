...
{
	private["_nonGroupUnits"];
	_nonGroupUnits = [];
	{
		if (isPlayer _x && (alive _x) && !(_x in (units (group player)))) then 
		{
			_nonGroupUnits pushBack _x;
		};
	}forEach playableUnits;
	[_nonGroupUnits,"nonGroupPlayers"] call _fn_displayGroupIcons;
